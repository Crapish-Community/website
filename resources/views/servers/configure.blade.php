            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('servers.processconfigure', $server->uuid) }}" enctype="multipart/form-data">
						...
						@php
							$serverOwner = (Auth::user()->id == $server->user->id);
						@endphp
						@if($serverOwner || (Auth::user()->admin && $revealIp))
							<div class="form-group row">
						...
						@if(!$serverOwner && Auth::user()->admin && !$revealIp)
							<div class="form-group row">
								...
								<div class="col-md-6">
									<button type="submit" class="btn btn-primary w-100" name="revealip" value="1">Unmask Server IP</button>
									<p class="mt-1 mb-0 text-muted text-center">Please note that unmasking a server IP will be noted in the admin logs. Only do it if you really need to.</p>
								</div>
							</div>
						@endif
