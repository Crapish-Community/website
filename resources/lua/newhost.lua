...
local function Spawn(cb) local cr = coroutine.create(cb) coroutine.resume(cr) return cr end
...
local whitelisted = {"jump", "jit", "throttle", "steer", "simulationradius"}
if not success then
	...
	if(child:GetPlayer()) then
		...
		child.NewFilter = function(item)
			local success, res = pcall(function()
				...
				if item and item.Name == "token" and item:IsA("BoolValue") and item.Parent == child:GetPlayer() then
		...
		child.PropertyFilter = function(item, prop)
			local success, res = pcall(function()
				...
				if item and item:IsA("Humanoid") then if game.Players:GetPlayerFromCharacter(item.Parent) == rep:GetPlayer() and whitelisted[prop:lower()] ~= nil then return Enum.FilterResult.Accepted end end
				return Enum.FilterResult.Rejected
			end)
			if success then
				return res
